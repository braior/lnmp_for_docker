...
$query = $_QFG['db']->query("SELECT package,tools,decorative FROM " . getTName("qqfarm_nc") . " where uid=" . $_QFG['uid']);
while($value = $_QFG['db']->fetch_array($query)) {
	$listup[] = $value;
}
$package = qf_decode($listup[0][package]);
$fertilizer = qf_decode($listup[0][tools]);
$decorative = qf_decode($listup[0][decorative]);

//提示信息
$levelup_arr = '{"title":"升级奖励","direction":"这么快就升级到' . $levelups[$levelup_exp][level] . '级了啊？真是神速，奖励你' . $levelups[$levelup_exp][eDesc] . '，快到背包里看看吧。","item":[{"eType":"' . $levelups[$levelup_exp][eType] . '","eParam":"' . $levelups[$levelup_exp][eParam] . '","eNum":"' . $levelups[$levelup_exp][eNum] . '","name":"' . $levelups[$levelup_exp][name] . '"}],"level":' . $levelups[$levelup_exp][level] . '}';

//更新数据库
$levelup_sql = "";
if(1 == $levelups[$levelup]['eType']) {
	$zhongzi = $levelups[$levelup_exp]['eParam'];
	$package[$zhongzi] = $package[$zhongzi] + $levelups[$levelup_exp]['eNum'];
	...
	foreach((array )$decorative as $item_type => $value) {
		foreach((array )$value as $key1 => $value1) {
			if($key1 == $decID) {
				$buyed = 1;
				$decorative[$decType][$decID][validtime] += $itemtype[$decID][itemValidTime];
			}
		}
	}
	if(!$buyed) {
		$decorative[$decType][$decID][status] = 0;
		$decorative[$decType][$decID][validtime] = $_QFG['timestamp'] + $itemtype[$decID][itemValidTime];
	}
	$levelup_sql = ",decorative='".qf_encode($decorative)."'";//装饰
} elseif(3 == $levelups[$levelup_exp][eType]) {
	$gongju = $levelups[$levelup_exp][eParam];
	$fertilizer[$gongju] = $fertilizer[$gongju] + $levelups[$levelup_exp][eNum];
	$levelup_sql = ",tools='".qf_encode($fertilizer)."'";//道具
}

$nextExp = qf_toExp(qf_toLevel($levelup_exp) + 1); //下一级别的经验值
$_QFG['db']->query("UPDATE " . getTName("qqfarm_nc") . " set levelup={$nextExp}{$levelup_sql} where uid=". $_QFG['uid']);
