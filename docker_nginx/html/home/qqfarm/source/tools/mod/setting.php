...
$query = $_QFG['db']->query("select tips FROM " . getTName("qqfarm_nc") . " where uid=" . $_QFG['uid']);
...
if($_GET['do'] == "save") {
	...
	if(array_key_exists($tip_type, $ncTips)) {
		...
		$result = $_QFG['db']->query("UPDATE " . getTName("qqfarm_nc") . " set tips ='" . qf_encode($Tips) . "' where uid=" . $_QFG['uid']);
		if($result) {
